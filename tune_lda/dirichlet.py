...
from numpy import asanyarray, ones, log, vstack, exp, asarray, zeros, isscalar, average
...
def mle(D, tol=1e-7, method='meanprecision', maxiter=None, weights=None):
    ...
    if method == 'meanprecision':
        return _meanprecision(D, tol=tol, maxiter=maxiter, weights=weights)
    else:
        return _fixedpoint(D, tol=tol, maxiter=maxiter, weights=weights)


def _fixedpoint(D, tol=1e-7, maxiter=None, weights=None):
    '''Simple fixed point iteration method for MLE of Dirichlet distribution'''
    N, K = D.shape
    logp = average(log(D), axis=0, weights=weights)
    a0 = _init_a(D, weights=weights)
    ...
    for i in xrange(maxiter):
        ...
        # if norm(a1-a0) < tol:
        if abs(_loglikelihood(logp, N, a1) - _loglikelihood(logp, N, a0)) < tol:  # much faster
            return a1
        a0 = a1
...
def _meanprecision(D, tol=1e-7, maxiter=None, weights=None):
...
def _fit_s(a0, logp, tol=1e-7, maxiter=1000):
...
def _init_a(D, weights=None):
    '''Initial guess for Dirichlet alpha parameters given data D'''
    E = average(D, axis=0, weights=weights)
    E2 = average(D**2, axis=0, weights=weights)
